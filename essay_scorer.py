"""
Автоматическая оценка мотивационных анкет кандидатов
=====================================================
Запуск:
    python essay_scorer.py

Входной файл:  tests.xlsx
Выходной файл: val_predictions.xlsx

Зависимости (без BERT):
    pip install pandas openpyxl scikit-learn numpy scipy
...
DATA_PATH   = "tests.xlsx"
OUTPUT_PATH = "val_predictions.xlsx"

threshold   = 6
fold_out    = 5
fold_in     = 3
ridge_alpha = [0.1, 1.0, 10.0, 100.0]
enseble     = (0.4, 0.6)  # Ridge, GBM

questions = [
    ("q1", "score1", "Q1: Почему именно вы?"),
    ("q2", "score2", "Q2: Применение знаний"),
    ("q3", "score3", "Q3: Задача сверх обязанностей"),
]

# ----- загрузка данных -------------------------------------------------------

df = pd.read_excel(DATA_PATH, engine="openpyxl").replace(
    r"^\s*([.\-*xX]|норм|Норм|НОРМ)\s*$", np.nan, regex=True
)
...
# ----- обучение: TF-IDF + ансамбль Ridge/GBM ---------------------------------

print("\n=== TF-IDF + Nested K-Fold (Ridge x 0.4 + GBM x 0.6) ===\n")
...
for q_col, s_col, q_name in questions:
    ...
    for fold, (tr_i, val_i) in enumerate(fold_outf.split(texts)):
        ...
        gbm.fit(X_tr, y_tr)
        preds = np.clip(
            enseble[0] * ridge.predict(X_val) + enseble[1] * gbm.predict(X_val), 0, 3
        )
        oof[val_i] = preds
    ...
    print(f"{q_name}")
    print(f"  MAE = {mean_absolute_error(y, oof):.3f}  QWK = {qwk(y, oof, 3):.3f}")
...
print(f"\nИтог (OOF)")
print(f"  MAE      = {mean_absolute_error(total_true, all_oof):.3f}")
print(f"  RMSE     = {mean_squared_error(total_true, all_oof) ** 0.5:.3f}")
print(f"  QWK      = {qwk(total_true, all_oof, 9):.3f}")
print(f"  Accuracy = {accuracy_score(pt, pp):.3f}  F1 = {f1_score(pt, pp, zero_division=0):.3f}")
...
for q_col, s_col, _ in questions:
    ...
    val_preds[s_col] = np.clip(
        enseble[0] * ridge_f.predict(X_v) + enseble[1] * gbm_f.predict(X_v), 0, 3
    )
...
# ----- BERT (опционально) ----------------------------------------------------
# Чтобы использовать BERT поставь USE_BERT = True и выбери модель.
# Требует: pip install transformers torch

USE_BERT   = False
BERT_MODEL = "cointegrated/rubert-tiny2"  # или "DeepPavlov/rubert-base-cased"
...
if USE_BERT:
    ...
    def encode_all_questions_bert(df: pd.DataFrame, model_name: str, device) -> np.ndarray:
        parts = []
        for q_col in ["q1", "q2", "q3"]:
            print(f"  кодируем {q_col}...")
            parts.append(get_bert_embeddings(df[q_col].tolist(), model_name, device))
        return np.hstack(parts)

    X_bert_train = encode_all_questions_bert(train, BERT_MODEL, device)
    X_bert_val   = encode_all_questions_bert(val,   BERT_MODEL, device)
    ...
    total_true = train["total"].fillna(0).values
    print(f"\nИтог BERT - MAE: {mean_absolute_error(total_true, bert_oof_total):.3f}  "
          f"QWK: {qwk(total_true, bert_oof_total, 9):.3f}")

    bert_val_preds = {}
    for q_col, s_col, _ in questions:
        bert_val_preds[s_col] = np.clip(bert_models[q_col].predict(X_bert_val), 0, 3)
    ...
    predictions["pass"] = (predictions["pred_total"] >= threshold).astype(int)
    OUTPUT_PATH = f"predictions_{BERT_MODEL.split('/')[-1]}.xlsx"

# ----- сохранение ------------------------------------------------------------

predictions.to_excel(OUTPUT_PATH, index=False)

print(f"\nПрошло порог (>= {threshold}): {predictions['pass'].mean():.1%} "
      f"({predictions['pass'].sum()} из {len(predictions)})")
